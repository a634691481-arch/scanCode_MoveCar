		...
		<view class="page-content">
			...
			<view class="info-card">
				...
				<view class="form-item">
					...
					<input v-model="form.carDesc" class="form-input" placeholder="例如：白色丰田凯美瑞" placeholder-class="form-placeholder"
						maxlength="20" />
				</view>
				...
				<view class="form-item">
					...
					<input v-model="form.phone" class="form-input" placeholder="请输入手机号" placeholder-class="form-placeholder"
				...
				<view class="form-item">
					...
					<input v-model="form.subPhone" class="form-input" placeholder="请输入备用手机号" placeholder-class="form-placeholder"
						type="number" maxlength="11" />
				</view>
				...
				<view class="form-item">
					...
					<textarea v-model="form.note" class="form-textarea" placeholder="例如：开会中不便接电话，稍后会回电"
						placeholder-class="form-placeholder" maxlength="80" :show-confirm-bar="false" auto-height />
				...
				<view v-if="form.receiveNotify" class="token-collapse">
					<view class="token-collapse-header" @click="tokenExpanded = !tokenExpanded">
						...
						<yy-icon
							:name="tokenExpanded ? 'ri:arrow-up-s-line' : 'ri:arrow-down-s-line'"
							size="18"
							color="#9ca3af"
						/>
					</view>
				...
				<!-- 保存按钮 -->
				<view class="save-btn" :class="{ 'save-btn-disabled': !canSave }" :style="canSave ? saveBtnStyle : {}"
					@click="saveInfo">
...
<script setup>
	...
	async function loadInfo() {
		const savedPlate = vk.getStorageSync('my_car_plate')
		if (!savedPlate) {
					...
					try {
						...
						if (savedPlate) {
							await vk.callFunction({
								url: 'client/pub_index.deleteCarInfo',
								data: { plate: savedPlate },
								needAlert: false,
							})
						}
