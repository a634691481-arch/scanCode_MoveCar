<template>
	<z-paging ref="paging" :auto="auto" :bgColor="bgColor" v-model="list" fixed refresher-threshold="160rpx"
		@query="queryList" @onRefresh="onRefresh" @scrolltolower="scrolltolower" @scrollIntoViewById="scrollIntoViewById"
		@scrollToTop="scrollToTop" :useVirtualList="useVirtualList" :useInnerList="useInnerList" :cellKeyName="cellKeyName"
		:innerListStyle="innerListStyle" :cellHeightMode="cellHeightMode" :virtualScrollFps="virtualScrollFps"
		:show-refresher-when-reload="showRefresherWhenReload" :loading-more-enabled="loadingMoreEnabled"
		:use-page-scroll="usePageScroll" created-reload :preloadPage="preloadPage" :refresherEnabled="refresherEnabled"
		scroll-with-animation :auto-hide-loading-after-first-loaded="true"
		:loading-more-loading-text="{ en: '英文的加载中', 'zh-cn': '中文的加载中', 'zh-hant-cn': '繁体的加载中' }" :empty-view-center="false"
		loading-full-fixed show-refresher-update-time efresher-angle-enable-change-continued auto-show-back-to-top
		:safe-area-inset-bottom="false" :use-safe-area-placeholder="false">
		<!-- refresher-vibrate -->
		<!--  -->
        ...
        ></u-navbar> -->
			...
			<u-navbar v-if="!hideNav" :is-back="showNavBack && !showTabbar" :title="navTitle" :background="navBackground"
				:is-fixed="true" :immersive="false" back-icon-name="arrow-leftward" title-width="350"
				:title-color="navTitleColor" :back-icon-color="backIconColor"></u-navbar>
			<slot name="top" />
			<!---->
		</template>
